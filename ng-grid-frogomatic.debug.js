...
var ngMoveSelectionHandler = function($scope, elm, evt, grid) {
    ...
    if (offset) {
        ...
        var rowHeight = r.rowHeight || $scope.rowHeight;        // VRH
...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', function($utils) {
    ...
    domUtilityService.UpdateGridLayout = function($scope, grid) {
        ...
        grid.elementDims.rootMaxW = grid.$root.width();         // FIXME - this is incorrect in Chrome!
        ...
        // Kludge for Chrome bug
        else {
            if (grid.elementDims.prevRootMaxW == grid.elementDims.rootMaxW + 20) {
                if (grid.maxCanvasHt <= $scope.viewportDimHeight()) {
                    //console.log("Chrome bug detected");
                    grid.elementDims.rootMaxW = grid.elementDims.prevRootMaxW;
                }
            }
        }
        grid.elementDims.prevRootMaxW = grid.elementDims.rootMaxW;
    ...
    domUtilityService.BuildStyles = function($scope, grid, digest) {
        // VRH row height shouldn't matter here, that's just the default
...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.enableCellEdit = colDef.enableCellEdit !== undefined ? colDef.enableCellEdit : (config.enableCellEdit || config.enableCellEditOnFocus || config.enableRowEdit);
    ...
    if(self.enableCellEdit) {
        if (config.enableRowEdit) {
            self.cellEditTemplate = $templateCache.get('cellRowEditTemplate.html');
            self.editableCellTemplate = colDef.editableCellTemplate || $templateCache.get('rowEditableCellTemplate.html');
        }
        else {
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q, $compile) {
    var defaults = {
        ...
        //Enables row-level cell editing.
        enableRowEdit: false,
        ...
        // Include a column for grouping
        addGroupingColumn: true,
        ...
        */
        ...
        // Height of the edit panel when editing a row
        rowEditPanelHeight: 40,
    ...
    self.initTemplates = function() {
        var templates = ['rowTemplate', 'editableRowTemplate', 'expandableRowTemplate', 'aggregateTemplate', 'headerRowTemplate', 'checkboxCellTemplate', 'checkboxHeaderTemplate', 'menuTemplate', 'footerTemplate'];
    ...
    self.minRowsToRender = function() {
        ...
        return Math.floor(viewportH / self.config.rowHeight);   // VRH TODO
    };
    self.refreshDomSizes = function() {
        ...
        if (dim.outerWidth > 50 && dim.outerHeight > 0) {
//            console.log("" + dim.outerWidth + " x " + dim.outerHeight);
            self.rootDim = dim;
        ...
        if (columnDefs.length > 0) {
            ...
            angular.forEach(columnDefs, function(colDef, i) {
                ...
                var column = new ngColumn({
                    ...
                    enableRowEdit: self.config.enableRowEdit,
    ...
    self.configureColumnWidths = function() {
        ...
        // ROWEX add in expander tab to total width?
        //if (self.config.enableRowExpansion) {
        //  totalWidth += (self.config.rowExpansionColumnWidth || 20);
        //}
    ...
    $scope.rowEditHeight = self.config.rowEditHeight;
    ...
    if (self.config.enableRowExpansion) {
        ...
        $scope.expandRow = function(row) {
            ...
            var templatePromise = options.template ?
    ...
    //Row editing functions (could probably be in a plugin)
    if (self.config.enableRowEdit) {
        $scope.editRow = function(row) {
            var oldRow = $scope.editingRow;
            //console.log("In edit function");
            //console.dir(row);

            if (oldRow !== row) {
                if (oldRow) {
                    $scope.cancelRowEdit(oldRow);
                }

                $scope.editingRow = row;
                
                row.isEditing = true;
                row.pushHeight('edit', row.rowEditPanelHeight || self.config.rowEditPanelHeight);

                self.rowFactory.renderedChange();

                row.originalEntity = angular.copy(row.entity);
            }
        };

        if (self.config.bindRowEdit) {
            self.config.bindRowEdit($scope.editRow);
        }

        $scope.saveRowEdit = function(row) {
            row.popHeight('edit');

            row.isEditing = false;
            $scope.editingRow = undefined;
            row.originalEntity = undefined;

            if (self.config.onRowSave) {
                self.config.onRowSave(row);
            }
        ...
        $scope.cancelRowEdit = function(row) {
            ...
            row.isEditing = false;
            $scope.editingRow = undefined;
            angular.forEach(row.originalEntity, function(val,key) {
            ...
            if (self.config.onRowCancel) {
                self.config.onRowCancel(row);
            }
    ...
    $scope.onDoubleClick = function(row) {
        if (typeof(self.config.onDoubleClick) != 'undefined') {
            self.config.onDoubleClick(row);
        }
    };


    $scope.updateFilter = function() {
        self.config.filterOptions.filterText = $scope.filterText;
    };
...
var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
	...
	this.afterSelectionChange = config.afterSelectionChangeCallback;
        this.rowHeight = config.rowHeight;                                  // VRH TODO
	this.offsetTop = this.rowIndex * this.rowHeight;                    // VRH TODO
    ...
    self.getGrouping = function(groups) {
        ...
        if(cols.length > 0) {
            for (var z = 0; z < groups.length; z++) {
                if (grid.config.addGroupingColumn && !cols[z].isAggCol && z <= maxDepth) {
...
var ngSelectionProvider = function (grid, $scope, $parse) {
    ...
    self.ChangeSelection = function (rowItem, evt) {
        ...
        else if (evt && !evt.keyCode && self.multi && grid.config.enableRowSelection) {
            // FROGOMATIC - require control key for selecting multiple rows
            if (grid.config.multiSelectWithCtrl && !evt.ctrlKey)
...
var ngStyleProvider = function($scope, grid) {
    ...
    $scope.rowStyle = function (row) {
        var height = row.rowHeight || $scope.rowHeight;
        var ret = { "top": row.offsetTop + "px", "height": height + "px" };
        if (row.isAggRow) {
...
/* FROGOMATIC - We don't actually want the edit cursor
            ...
            return {
                pre: function($scope, iElement) {
                    ...
                    if ($scope.row.isAggRow) {
                        ...
                        var template;
                        if ($scope.editRow) {
                            template = 'editableRowTemplate.html';
                        }
                        else if ($scope.expandRow) {
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("cellRowEditTemplate.html",
    "<div> " +
    "	<div ng-edit-cell-if=\"!row.isEditing\">	" +
  ...
  // FIXME - I shouldn't be specifying line height explicitly here
  $templateCache.put("rowEditableCellTemplate.html",
    "<input ng-class=\"'colt' + col.index\" ng-model=\"COL_FIELD\" style=\"height: 22px; line-height: 20px;\" />"
  );
  ...
  $templateCache.put("gridTemplate.html",
    ...
    "        <div ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\" ng-dblclick=\"onDoubleClick(row)\" ng-click=\"row.toggleSelected($event)\" ng-class=\"row.alternatingRowClass()\" ng-row></div>" +
    "    </div>" +
    "</div>" +
    "<div ng-grid-footer></div>" +
    ""
  ); // FIXME - probably don't want ng-dblclick there universally
  ...
  $templateCache.put("menuTemplate.html",
    ...
    "        <button class='btn add-on' ng-click=\"updateFilter()\"><i class='icon-search'></i></button>" +
    "    </div>" +
  ...
  $templateCache.put("editableRowTemplate.html",
    '<div ng-dblclick=\"editRow(row)\" style="background-color: #ccc">' +
        '<div ng-edit-cell-if="!row.isEditing">' +
            "<div ng-style=\"{ 'cursor': row.cursor }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\">" +
            "	<div class=\"ngVerticalBar\" ng-style=\"{height: rowHeight}\" ng-class=\"{ ngVerticalBarVisible: !$last }\">&nbsp;</div>" +
            "	<div ng-cell></div>" +
        ...
        '</div>' +
        '<div ng-edit-cell-if="row.isEditing">' +
            "<div ng-style='{ height: rowEditHeight, \"margin-bottom\": \"1px\" }'>" +
                "<div ng-style=\"{ 'cursor': row.cursor, 'height': rowEditHeight }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\" >" +
                "	<div class=\"ngVerticalBar\" ng-style=\"{ height: rowEditHeight }\" ng-class=\"{ ngVerticalBarVisible: !$last }\">&nbsp;</div>" +
                "	<div ng-cell></div>" +
                "</div>" +
            "</div>" + 
            '<div style="text-align: center; background-color: #fff; border: 1px solid #ccc; height: 30px; padding: 3px; padding-top: 5px;">' +
                '<button class="btn btn-small toolbutton" ng-click="saveRowEdit(row)" style="margin-right: 10px"><i class="getit-icon-ok"></i>Save</button>' +
                '<button class="btn btn-small toolbutton" ng-click="cancelRowEdit(row)" style="margin-left: 10px"><i class="getit-icon-cancel"></i>Cancel</button>' + 
            "</div>" + 
        '</div>' +
    '</div>'
  ...
  $templateCache.put("expandableRowTemplate.html",
    '<div>' +
        '<div ng-show="row.isExpanded" class="ngCell ngCellExpander expanded" style="width: 15px">' +
            '<a ng-click="collapseRow(row)"><i class="icon-collapse" style="float: left; padding-left: 3px; padding-top: 3px; font-size: 13px" ></i></a>' +
        '</div>' +
        '<div ng-show="!row.isExpanded" class="ngCell ngellExpander collapsed">' +
            '<a ng-click="expandRow(row)"><i class="icon-expand" style="float: left; padding-left: 3px; padding-top: 3px; font-size: 13px" ></i></a>' +
        '</div>' +
